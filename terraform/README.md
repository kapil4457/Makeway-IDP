...
| Variable | Meaning |
|---|---|
| `kube_api_endpoint` / `kube_ca_cert` / `kube_token` | Fallback kube route when a claim/cluster has no registry creds (e.g. the localtunnel-exposed local cluster — localTunnel/README.md); per-capability creds otherwise come from the control-plane Cluster registry. `kube_ca_cert` stays empty for the dev setup |
